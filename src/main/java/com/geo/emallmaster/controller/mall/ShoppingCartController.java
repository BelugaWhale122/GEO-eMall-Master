...
public class ShoppingCartController {
    ...
    public String cartListPage(HttpServletRequest request,
        ...
        int itemsTotal = 0;
        int priceTotal = 0;
        List<ShoppingCartItemVO> myShoppingCartItems = shoppingCarService.getMyShoppingCartItems(user.getUserId());
    ...
    public Result saveShoppingCartItem(@RequestBody ShoppingCartItem shoppingCartItem,
        ...
        String saveResult = shoppingCarService.saveShoppingCartItem(shoppingCartItem);
        //添加成功
    ...
    public Result updateShoppingCartItem(@RequestBody ShoppingCartItem shoppingCartItem,
        ...
        UserVO user = (UserVO) httpSession.getAttribute(Constants.USER_SESSION_KEY);
        shoppingCartItem.setUserId(user.getUserId());
        String updateResult = shoppingCarService.updateShoppingCartItem(shoppingCartItem);
        //修改成功
    ...
    public Result updateShoppingCartItem(@PathVariable("shoppingCartItemId") Long shoppingCartItemId,
                                         HttpSession httpSession) {
        UserVO user = (UserVO) httpSession.getAttribute(Constants.USER_SESSION_KEY);
        Boolean deleteResult = shoppingCarService.deleteById(shoppingCartItemId, user.getUserId());
        //删除成功
        if (deleteResult) {
            return ResultGenerator.genSuccessResult();
        }
